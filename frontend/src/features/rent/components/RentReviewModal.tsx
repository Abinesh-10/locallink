...
export function RentReviewModal({ listingId, bookingId, onClose, onSuccess }: RentReviewModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setIsLoading(true);
    try {
      await rentApi.createReview(listingId, bookingId, rating, comment || undefined);
      onSuccess();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink-900/40 sm:items-center" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-display text-lg font-semibold text-ink-900">{t('rent.leaveReview')}</h2>
