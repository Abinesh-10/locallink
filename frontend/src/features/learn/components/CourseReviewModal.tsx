...
export function CourseReviewModal({ courseId, onClose, onSuccess }: CourseReviewModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setIsLoading(true);
    try {
      await learnApi.createReview(courseId, rating, comment || undefined);
      onSuccess();
    } catch (err: any) {
      if (err.response?.data?.title === 'enrollment_required') {
        setError(t('learn.enrollmentRequiredError'));
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink-900/40 sm:items-center" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-display text-lg font-semibold text-ink-900">{t('learn.leaveReview')}</h2>
